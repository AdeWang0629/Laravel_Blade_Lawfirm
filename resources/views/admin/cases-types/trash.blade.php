...
@section('title', removebeginninLetters(trans_choice('site.trashes', 1), 2) .' '. removebeginninLetters(trans_choice('site.categories', 1), 2) .' '. trans_choice('site.lawsuites', 1))
...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">{{ trans('site.table') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ removebeginninLetters(trans_choice('site.trashes', 1), 2) .' '. removebeginninLetters(trans_choice('site.categories', 1), 2) .' '. trans_choice('site.lawsuites', 1) }}</span>
			</div>
		</div>
		...
		<div class="d-flex my-xl-auto right-content">
            <div class="pr-1 mb-3 mb-xl-0">
                @can('caseType_list')
                    <a href="{{ route('admin.case-types.index') }}" class="btn btn-sm btn-success">{{ trans('site.all_attr', ['attr' => trans_choice('site.categories', 1)]) }} <i class="fas fa-long-arrow-alt-left"></i></a>
                @else
                    <button type="button" class="btn btn-sm btn-success disabled">{{ trans('site.all_attr', ['attr' => trans_choice('site.categories', 1)]) }} <i class="fas fa-long-arrow-alt-left"></i></button>
                @endcan
            </div>
		</div>
	</div>
					...
					<div class="card mg-b-20">
						<div class="card-header pb-0">
							<div class="d-flex justify-content-between">
								<h4 class="card-title mg-b-0">{{ removebeginninLetters(trans_choice('site.trashes', 1), 2) .' '. removebeginninLetters(trans_choice('site.categories', 1), 2) .' '. trans_choice('site.lawsuites', 1) }}</h4>
										...
										<tr>
											...
											<td>
												@can('caseType_delete')
													...
													data-url="{{ route('admin.case-types.force.delete', $caseType) }}"
													data-id="{{ $caseType->id }}"
													data-delete_label="{{ trans('site.sure_force_delete_attr', ['attr' => $caseType->name]) }}"
                                                ...
                                                @can('caseType_restore')
													...
													data-url="{{ route('admin.case-types.restore', $caseType) }}"
													data-id="{{ $caseType->id }}"
													data-restore_label="{{ trans('site.sure_restore_attr', ['attr' => $caseType->name]) }}"
											...
											<td>{{ $caseType->deleted_at->translatedFormat('d M, Y') }}</td>
										</tr>
										@endforeach
									</tbody>
								</table>
