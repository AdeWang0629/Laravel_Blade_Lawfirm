...
@section('title', removebeginninLetters(trans_choice('site.categories', 1), 2) .' '. trans_choice('site.clients', 1))
...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">{{ trans('site.table') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ removebeginninLetters(trans_choice('site.categories', 1), 2) .' '. trans_choice('site.clients', 1) }}</span>
			</div>
		</div>
            ...
            <div class="pr-1 mb-3 mb-xl-0">
                @if ($trashed->count() > 0)
                    @can('clientType_delete')
                        <a class="btn btn-sm btn-danger" href="{{ route('admin.clients-types.trashed') }}"><i class="mdi mdi-delete-sweep"></i> {{ trans('site.trashes') }}</a>
                    @else
						...
						<div class="card mg-b-20">
							<div class="card-header pb-0">
								<div class="d-flex justify-content-between">
									<h4 class="card-title mg-b-0">{{ removebeginninLetters(trans_choice('site.categories', 1), 2) .' '. trans_choice('site.clients', 1)}}</h4>
									<i class="mdi mdi-dots-horizontal text-gray"></i>
								</div>
								<p class="tx-12 tx-gray-500 mb-2">{{ trans('site.here_you_can',['attr' => removebeginninLetters(trans_choice('site.categories', 1), 2) .' '. trans_choice('site.clients', 1)]) }}...
									@can('clientType_create')
										<button type="button" class="btn btn-link btn-sm p-0 modal-effect"  data-effect="effect-slide-in-right" data-toggle="modal" data-target="#clientType"
										data-url="{{ route('admin.clients-types.store') }}"
										data-submit_btn="{{ trans('site.add') }}"
										data-label_name="{{ trans('site.name') }}"
										data-modal_title="{{ trans('site.add_new', ['attr' => removebeginninLetters(trans_choice('site.categories', 0), 2)]) }}"> {{ trans('site.add_new', ['attr' => removebeginninLetters(trans_choice('site.categories', 0), 2)]) }}</button>
									@endcan
								</p>
							</div>
							<div class="card-body">
											...
											<tr>
												...
												<td>
													@can('clientType_edit')
														...
														data-url="{{ route('admin.clients-types.update', $clientType) }}"
														data-client_type_name="{{ $clientType->name }}"
													...
													@can('clientType_delete')
														...
														data-url="{{ route('admin.clients-types.destroy', $clientType) }}"
														data-id="{{ $clientType->id }}"
														data-delete_label="{{ trans('site.sure_delete_attr', ['attr' => $clientType->name]) }}"
												...
												<td>{{ $clientType->created_at->translatedFormat('d M, Y') }}</td>
											</tr>
											@endforeach
										</tbody>
									</table>
	...
	$(function() {
		...
		@canany(['clientType_create', 'clientType_edit'])
			...
			$('#clientType').on('shown.bs.modal', function(event) {
				...
				url = button.data('url'),
				client_type_name = button.data('client_type_name'),
				...
				modal.find('#client_type_name').val(client_type_name)
