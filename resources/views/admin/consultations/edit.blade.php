...
@section('title', trans('site.edit', ['attr' => trans_choice('site.consultations', 0)]))
...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">{{ trans('site.home') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('site.edit', ['attr' => trans_choice('site.consultations', 0)]) }}</span>
			</div>
		</div>
						...
						<div class="card-body pt-0">
							<div class="row">
								<div class="form-group col-lg">
									...
									<select class="form-control @error('client_id') is-invalid @enderror clients" name="client_id">
										...
										@foreach ($clients as $client)
											<option value="{{ $client->id }}" {{ old('client_id', $consultation->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
										@endforeach
									</select>
									@error('client_id')
								...
								<div class="form-group col-lg">
									...
									<input type="text" class="form-control @error('consultation_subject') is-invalid @enderror" name="consultation_subject" id="consultation_subject" placeholder="{{ removebeginninLetters(trans('site.subject'), 2) . ' ' . trans_choice('site.consultations', 0) }}" value="{{ old('consultation_subject', $consultation->consultation_subject) }}">
								...
								<div class="form-group col-lg">
									...
									<div class="input-group">
										...
										</div><input class="form-control @error('contract_date') is-invalid @enderror fc-datepicker" name="contract_date" placeholder="MM/DD/YYYY" type="text" value="{{ old('contract_date', $consultation->contract_date->format('Y-m-d') ?? now()->format('Y-m-d')) }}">
									</div>
									@error('contract_date')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
								...
								<div class="form-group col-lg">
									...
									<input type="number" min="0" step="any" class="form-control @error('contract_amount') is-invalid @enderror" name="contract_amount" id="contract_amount" placeholder="{{ trans('site.amount_attr', ['attr' => trans_choice('site.contracts', 0)]) }}" value="{{ old('contract_amount', $consultation->contract_amount) }}">
								...
								<div class="form-group col-lg">
									...
									<input type="number" min="0" step="any" class="form-control @error('vat') is-invalid @enderror" name="vat" id="vat" placeholder="{{ trans('site.amount_attr', ['attr' => trans('site.vat')]) }}" value="{{ old('vat', $consultation->vat ?? getSettingOf('vat')) }}">
									@error('vat')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
								...
								<div class="form-group col-lg">
									...
									<input type="number" min="0" step="any" class="form-control @error('total_amount') is-invalid @enderror" name="total_amount" id="total_amount" placeholder="{{ trans('site.contract_amount_including_tax') }}" value="{{ old('total_amount', $consultation->total_amount) }}" readonly>
									@error('total_amount')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
						...
						<div class="card-body pt-0">
							<div class="row">
								...
								<div class="form-group col-lg">
									...
									<textarea name="contract_terms" class="form-control @error('contract_terms') is-invalid @enderror contract_textarea" name="contract_terms" id="contract_terms" placeholder="{{ trans('site.contract_terms') }}">{{ old('contract_terms', $consultation->contract_terms) }}</textarea>
