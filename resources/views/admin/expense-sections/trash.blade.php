...
@section('title', removebeginninLetters(trans_choice('site.trashes', 1), 2) .' '. removebeginninLetters(trans_choice('site.sections', 1), 2).' '.trans_choice('site.expenses', 1))
...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">{{ trans('site.table') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ removebeginninLetters(trans_choice('site.trashes', 1), 2) .' '. removebeginninLetters(trans_choice('site.sections', 1), 2).' '.trans_choice('site.expenses', 1) }}</span>
			</div>
		</div>
		...
		<div class="d-flex my-xl-auto right-content">
            <div class="pr-1 mb-3 mb-xl-0">
                @can('expenseSection_list')
                    <a href="{{ route('admin.expense-sections.index') }}" class="btn btn-sm btn-success">{{ trans('site.all_attr', ['attr' =>  removebeginninLetters(trans_choice('site.sections', 1), 2).' '.trans_choice('site.expenses', 1)]) }} <i class="fas fa-long-arrow-alt-left"></i></a>
                @else
                    <button type="button" class="btn btn-sm btn-success disabled">{{ trans('site.all_attr', ['attr' =>  removebeginninLetters(trans_choice('site.sections', 1), 2).' '.trans_choice('site.expenses', 1)]) }} <i class="fas fa-long-arrow-alt-left"></i></button>
                @endcan
            </div>
        </div>
	</div>
                    ...
                    <div class="card mg-b-20">
                        <div class="card-header pb-0">
                            <div class="d-flex justify-content-between">
                                <h4 class="card-title mg-b-0">{{ removebeginninLetters(trans_choice('site.trashes', 1), 2) .' '. removebeginninLetters(trans_choice('site.sections', 1), 2).' '.trans_choice('site.expenses', 1) }}</h4>
                        ...
                        <div class="card-body">
                            @can('expenseSection_list')
                                <table id="example" class="table table-striped dt-responsive text-center mg-b-0 text-md-nowrap" style="width:100%">
                                    ...
                                    <tbody>
                                        @foreach ($trashed as $expense_sections)
                                        <tr>
                                            ...
                                            <td>
                                                @can('expenseSection_delete')
                                                    ...
                                                    data-url="{{ route('admin.expense-sections.force.delete', $expense_sections) }}"
                                                    data-id="{{ $expense_sections->id }}"
                                                    data-delete_label="{{ trans('site.sure_force_delete_attr', ['attr' => $expense_sections->name]) }}"
                                                ...
                                                @can('expenseSection_restore')
                                                    ...
                                                    data-url="{{ route('admin.branches.restore', $expense_sections) }}"
                                                    data-id="{{ $expense_sections->id }}"
                                                    data-restore_label="{{ trans('site.sure_restore_attr', ['attr' => $expense_sections->name]) }}"
                                            ...
                                            <td>{{ $expense_sections->deleted_at->translatedFormat('d M, Y') }}</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
