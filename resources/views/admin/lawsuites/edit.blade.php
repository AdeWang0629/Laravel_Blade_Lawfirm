...
@section('title', trans('site.edit', ['attr' => removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]))
...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">{{ trans('site.home') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('site.edit', ['attr' => removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}</span>
			</div>
		</div>
...
@section('content')
		...
		<form action="{{ route('admin.lawsuites.update', $lawsuite) }}" method="POST">
			@csrf
			@method('PUT')
								...
								<div class="form-group col-lg">
									...
									<select class="form-control @error('client_id') is-invalid @enderror clients" name="client_id">
										...
										@foreach ($clients as $client)
											<option value="{{ $client->id }}" {{ old('client_id', $lawsuite->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
										@endforeach
									</select>
									@error('client_id')
								...
								<div class="form-group col-lg">
									...
									<select class="form-control @error('client_type_id') is-invalid @enderror clients" name="client_type_id">
										...
										@foreach ($clientTypes as $clientType)
											<option value="{{ $clientType->id }}" {{ old('client_type_id', $lawsuite->client_type_id) == $clientType->id ? 'selected' : '' }}>{{ $clientType->name }}</option>
										@endforeach
									</select>
						...
						<div class="card-body pt-0">
							<div class="row">
								<div class="form-group col-lg">
									...
									<select class="form-control @error('case_type_id') is-invalid @enderror clients" name="case_type_id">
										...
										@foreach ($caseTypes as $caseType)
											<option value="{{ $caseType->id }}" {{ old('case_type_id', $lawsuite->case_type_id) == $caseType->id ? 'selected' : '' }}>{{ $caseType->name }}</option>
										@endforeach
									</select>
									@error('case_type_id')
										<span class="invalid-feedback" role="alert">
								...
								<div class="form-group col-lg">
									...
									<input type="text" class="form-control @error('lawsuite_lawyer') is-invalid @enderror" name="lawsuite_lawyer" id="lawsuite_lawyer" placeholder="{{ trans('site.lawyer_attr', ['attr' => trans_choice('site.lawsuites', 0)]) }}" value="{{ old('lawsuite_lawyer', $lawsuite->lawsuite_lawyer) }}">
								...
								<div class="form-group col-lg-6">
									...
									<input type="text" class="form-control @error('lawsuite_subject') is-invalid @enderror" name="lawsuite_subject" id="lawsuite_subject" placeholder="{{ removebeginninLetters(trans('site.subject'), 2) . ' ' . trans_choice('site.lawsuites', 0) }}" value="{{ old('lawsuite_subject', $lawsuite->lawsuite_subject) }}">
								...
								<div class="form-group col-lg">
									...
									<select class="form-control @error('case_stage_id') is-invalid @enderror clients" name="case_stage_id">
										...
										@foreach ($caseStages as $caseStage)
											<option value="{{ $caseStage->id }}" {{ old('case_stage_id', $lawsuite->case_stage_id) == $caseStage->id ? 'selected' : '' }}>{{ $caseStage->name }}</option>
										@endforeach
									</select>
									@error('case_stage_id')
										<span class="invalid-feedback" role="alert">
								...
								<div class="form-group col-lg">
									...
									<select class="form-control @error('lawsuit_case_id') is-invalid @enderror clients" name="lawsuit_case_id">
										...
										@foreach ($lawsuiteCases as $lawsuiteCase)
											<option value="{{ $lawsuiteCase->id }}" {{ old('lawsuit_case_id', $lawsuite->lawsuit_case_id) == $lawsuiteCase->id ? 'selected' : '' }}>{{ $lawsuiteCase->name }}</option>
										@endforeach
									</select>
								...
								<div class="form-group col-lg">
									...
									<select class="form-control @error('court_id') is-invalid @enderror clients" name="court_id">
										...
										@foreach ($courts as $court)
											<option value="{{ $court->id }}" {{ old('court_id', $lawsuite->court_id) == $court->id ? 'selected' : '' }}>{{ $court->name }}</option>
										@endforeach
									</select>
									@error('court_id')
										<span class="invalid-feedback" role="alert">
								...
								<div class="form-group col-lg">
									...
									<input type="text" class="form-control @error('court_case_number') is-invalid @enderror" name="court_case_number" id="court_case_number" placeholder="{{ trans('site.court_lawsuite_number') }}" value="{{ old('court_case_number', $lawsuite->court_case_number) }}">
						...
						<div class="card-body pt-0">
							<div class="row">
                                <div class="form-group col-lg">
									...
									<input type="text" class="form-control @error('contract_title') is-invalid @enderror" name="contract_title" id="contract_title" placeholder="{{ trans('site.title') }}" value="{{ old('contract_title', $lawsuite->contract_title ?? getSettingOf('contract_title') ) }}">
							...
							<div class="row">
								<div class="form-group col-lg">
									...
									<div class="input-group">
										...
										</div><input class="form-control @error('contract_date') is-invalid @enderror fc-datepicker" name="contract_date" placeholder="MM/DD/YYYY" type="text" value="{{ old('contract_date',  $lawsuite->contract_date->format('Y-m-d') ?? now()->format('Y-m-d')) }}">
									</div>
									@error('contract_date')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
								...
								<div class="form-group col-lg">
									...
									<input type="number" min="0" step="any" class="form-control @error('contract_amount') is-invalid @enderror" name="contract_amount" id="contract_amount" placeholder="{{ trans('site.amount_attr', ['attr' => trans_choice('site.contracts', 0)]) }}" value="{{ old('contract_amount',  $lawsuite->contract_amount) }}">
								...
								<div class="form-group col-lg">
									...
									<input type="number" min="0" step="any" class="form-control @error('vat') is-invalid @enderror" name="vat" id="vat" placeholder="{{ trans('site.amount_attr', ['attr' => trans('site.vat')]) }}" value="{{ old('vat',  $lawsuite->vat ?? getSettingOf('vat')) }}">
									@error('vat')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
								...
								<div class="form-group col-lg">
									...
									<input type="number" min="0" step="any" class="form-control @error('total_amount') is-invalid @enderror" name="total_amount" id="total_amount" placeholder="{{ trans('site.contract_amount_including_tax') }}" value="{{ old('total_amount',  $lawsuite->total_amount) }}" readonly>
									@error('total_amount')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
								...
								<div class="form-group col-lg">
									...
									<textarea name="contract_terms" class="form-control @error('contract_terms') is-invalid @enderror contract_textarea" name="contract_terms" id="contract_terms" placeholder="{{ trans('site.contract_terms') }}">{{ old('contract_terms',  $lawsuite->contract_terms) }}</textarea>
								...
								<div class="form-group col-lg">
									...
									<textarea class="form-control @error('notes') is-invalid @enderror" name="notes" id="notes" rows="4" placeholder="{{ trans('site.notes') }}">{{ old('notes',  $lawsuite->notes) }}</textarea>
									@error('notes')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
...
@section('js')
	...
	<script>
		$(function() {
			...
			var repeater = $('.repeater, .repeater-file, .repeater-add').repeater({
			...
			@if ($lawsuite->opponents->count()>0)
                ...
                @else
                    repeater.setList(@json($lawsuiteOpponents));
                @endif
            @endif
