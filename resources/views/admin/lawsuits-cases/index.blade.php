...
@section('title', trans_choice('site.status', 1) .' '. trans_choice('site.lawsuites', 1))
...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">{{ trans('site.table') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans_choice('site.status', 1) .' '. trans_choice('site.lawsuites', 1) }}</span>
			</div>
		</div>
		<div class="d-flex my-xl-auto right-content">
            <div class="pr-1 mb-3 mb-xl-0">
                @if ($trashed->count() > 0)
                    @can('lawsuitCase_delete')
                        <a class="btn btn-sm btn-danger" href="{{ route('admin.lawsuit-cases.trashed') }}"><i class="mdi mdi-delete-sweep"></i> {{ trans('site.trashes') }}</a>
                    @else
			...
			<div class="pr-1 mb-3 mb-xl-0">
				@can('lawsuitCase_create')
					<button type="button" class="btn btn-sm btn-success modal-effect"  data-effect="effect-slide-in-right" data-toggle="modal" data-target="#lawsuitCase"
					data-url="{{ route('admin.lawsuit-cases.store') }}"
					data-submit_btn="{{ trans('site.add') }}"
					data-modal_title="{{ trans('site.add_new', ['attr' => trans_choice('site.status', 0) .' '. removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}"
					data-method=""><i class="mdi mdi-plus"></i> {{ trans('site.add_new', ['attr' => trans_choice('site.status', 0) .' '. removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}</button>
				@else
					<button type="button" class="btn btn-sm btn-success disabled"><i class="mdi mdi-plus"></i> {{ trans('site.add_new', ['attr' => trans_choice('site.status', 0) .' '. removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}</button>
				@endcan
			</div>
		</div>
	</div>
...
@endsection
@section('content')
	@canany(['lawsuitCase_list', 'lawsuitCase_create', 'lawsuitCase_edit', 'lawsuitCase_delete'])
				<!-- row opened -->
						...
						<div class="card mg-b-20">
							<div class="card-header pb-0">
								<div class="d-flex justify-content-between">
									<h4 class="card-title mg-b-0">{{ trans_choice('site.status', 1) .' '. trans_choice('site.lawsuites', 1) }}</h4>
									<i class="mdi mdi-dots-horizontal text-gray"></i>
								</div>
								<p class="tx-12 tx-gray-500 mb-2">{{ trans('site.here_you_can',['attr' => trans_choice('site.status', 1) .' '. trans_choice('site.lawsuites', 1)]) }}...
									@can('lawsuitCase_create')
										<button type="button" class="btn btn-link btn-sm p-0 modal-effect"  data-effect="effect-slide-in-right" data-toggle="modal" data-target="#lawsuitCase"
										data-url="{{ route('admin.lawsuit-cases.store') }}"
										data-submit_btn="{{ trans('site.add') }}"
										data-modal_title="{{ trans('site.add_new', ['attr' => trans_choice('site.status', 0) .' '. removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}"
										data-method=""> {{ trans('site.add_new', ['attr' => trans_choice('site.status', 0) .' '. removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}</button>
									@endcan
								</p>
							</div>
							<div class="card-body">
								@can('lawsuitCase_list')
									<table id="example" class="table table-striped dt-responsive text-center mg-b-0 text-md-nowrap" style="width:100%">
										...
										<tbody>
											@foreach ($lawsuitCases as $lawsuitCase)
											<tr>
												...
												<td>
													@can('lawsuitCase_edit')
														<button type="button" class="btn btn-sm btn-info modal-effect"  data-effect="effect-slide-in-right" data-toggle="modal" data-target="#lawsuitCase"
														data-url="{{ route('admin.lawsuit-cases.update', $lawsuitCase) }}"
														data-id="{{ $lawsuitCase->id }}"
														data-lawsuite_case="{{ $lawsuitCase->name }}"
														data-color="{{ $lawsuitCase->color }}"
														data-submit_btn="{{ trans('site.update') }}"
														data-modal_title="{{ trans('site.update_attr', ['attr' => trans_choice('site.status', 0) .' '. removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}"
													...
													@can('lawsuitCase_delete')
														...
														data-url="{{ route('admin.lawsuit-cases.destroy', $lawsuitCase) }}"
														data-id="{{ $lawsuitCase->id }}"
														data-delete_label="{{ trans('site.sure_delete_attr', ['attr' => $lawsuitCase->name]) }}"
														data-modal_title="{{ trans('site.delete_attr', ['attr' => trans_choice('site.status', 0) .' '. removebeginninLetters(trans_choice('site.lawsuites', 0), 2)]) }}"><i class="las la-trash"></i></button>
													@else
														<button type="button" class="btn btn-sm btn-danger disabled"><i class="las la-trash"></i></button>
													@endcan
												</td>
												<td>{{ $lawsuitCase->created_at->translatedFormat('d M, Y') }}</td>
											</tr>
											@endforeach
										</tbody>
									</table>
		...
		@canany(['lawsuitCase_create', 'lawsuitCase_edit'])
			@include('admin.lawsuits-cases.partials.modal')
		@endcanany
		@can('lawsuitCase_delete')
...
<script>
	$(function(e) {
		...
		@canany(['lawsuitCase_create', 'lawsuitCase_edit'])
			...
			$('#lawsuitCase').on('shown.bs.modal', function(event) {
				...
				lawsuite_case = button.data('lawsuite_case'),
				id = button.data('id'),

				modal = $(this);
				modal.find('#lawsuite_case').focus();
				modal.find('input[name=_method]').val(method)
				modal.find('input[name=id]').val(id)
				modal.find('.modal-content').find('form').attr('action', url)
				modal.find('.modal-title').text(modal_title)
				modal.find('#lawsuite_case').val(lawsuite_case)
				...
				$('#colorpicker').spectrum({
					...
					change: function (color) {
						$(this).closest('col-lg').find('#colorpicker').val(color.toHex());
					}
				});
			});
		@endcanany
