...
@section('title', removebeginninLetters(trans_choice('site.reports', 1), 2) .' '. removebeginninLetters(trans_choice('site.payments', 1), 2) .' '.trans_choice('site.consultations', 1))
...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{ trans('site.table') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ removebeginninLetters(trans_choice('site.reports', 1), 2) .' '. removebeginninLetters(trans_choice('site.payments', 1), 2) .' '.trans_choice('site.consultations', 1) }}</span>
            </div>
        </div>
						...
						<div class="card mg-b-20">
							<div class="card-header pb-0">
								<div class="d-flex justify-content-between">
									<h4 class="card-title mg-b-0">{{ removebeginninLetters(trans_choice('site.reports', 1), 2) .' '. removebeginninLetters(trans_choice('site.payments', 1), 2) .' '.trans_choice('site.consultations', 1) }}</h4>
									<i class="mdi mdi-dots-horizontal text-gray"></i>
								</div>
							</div>
                            
							<div class="card-body">
                                <form action="{{ route('admin.consultations.payments.reports') }}" method="GET">
                                    <div class="row">
                                ...
                                <p class="case-info font-weight-bold">{{ trans('site.total_attr_by_date', ['attr' => trans_choice('site.consultations', 1)]) }}: <span class="text-danger">{{ $receipts->sum('debit') }}</span></p>
								<table id="example" class="table table-striped dt-responsive mg-b-0 text-md-nowrap" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>                                                
                                            ...
                                            @can('consultation_showContract')
                                                <td>
                                                    <a href="{{ route('admin.show.consultation.contract', $receipt->consultation_id) }}" target="_blank" class="btn btn-sm btn-success"><i class="fas fa-file-contract"></i></a>
                                                </td>
                                            @endcan
                                        </tr>
                                        @endforeach
