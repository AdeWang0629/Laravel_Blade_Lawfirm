...
@section('title', trans('site.receipt_number') .' '.$receipt->receipt_number)
...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto">{{ trans('site.home') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ trans('site.receipt_number') .' '.$receipt->receipt_number }}</span>
		</div>
	</div>
</div>
<!-- breadcrumb -->
                            ...
                            <div class="myDivToPrint">
                                ...
                                <div class="row mg-t-20 header-top">
                                    <div class="col-md text-center w-100">
                                        <p class="tx-gray font-weight-bold tx-16-f">سند قبض - Receipt voucher</p>
                                    </div>
                                </div>
                                <div class="row mg-t-20 header-top">
                                    <div class="col-md text-right  w-40">
                                        <p class="tx-gray-900 mb-0">{{ trans('site.receipt_date') }}: {{ $receipt->date }}</p>
                                    </div>
                                    <div class="col-md text-center w-20">
                                        <p class="font-weight-bold tx-20-f text-danger mb-0">{{ $receipt->receipt_number }}</p>
                                    </div>
                                ...
                                <div class="mg-t-20">
                                    <table class="table table-bordered" cellpadding="0" cellspacing="0">
                                        <tr>
                                            <td class="text-right">استلمنا من</td>
                                            <td class="text-center font-weight-bold">{{ $receipt->client->name }}</td>
                                            <td class="text-left">We received from</td>
                                        </tr>
                                        <tr>
                                            <td class="text-right">مبلغ وقدرة</td>
                                            <td class="text-center font-weight-bold">{{ currency_details($receipt->debit) }}</td>
                                            <td class="text-left">Amount of</td>
                                        </tr>
                                        <tr>
                                            <td class="text-right">طريقة الدفع</td>
                                            <td class="text-center font-weight-bold">{{ $receipt->payment_type }}</td>
                                            <td class="text-left">Payment method</td>
                                        </tr>
                                        <tr>
                                            <td class="text-right">وذلك عن</td>
                                            <td class="text-center font-weight-bold">{{ $receipt->note }}</td>
                                            <td class="text-left">That's about</td>
                                        </tr>
                                        @if ($receipt->lawsuite != null && $receipt->consultation == null)
                                            <tr>
                                                <td class="text-right">المبلغ الباقى وقت الطباعة</td>
                                                
                                                <td class="text-center font-weight-bold">{{ currency_details($receipt->lawsuite->lawsuiteRemaining) }}</td>
                                                <td class="text-left">Remaining amount at the time of printing</td>
                                            </tr>
                                        @endif
                                        @if ($receipt->consultation != null && $receipt->lawsuite == null)
                                            <tr>
                                                <td class="text-right">المبلغ الباقى وقت الطباعة</td>
                                                <td class="text-center font-weight-bold">{{ currency_details($receipt->consultation->consultationRemaining) }}</td>
                                                <td class="text-left">Remaining amount at the time of printing</td>
                                            </tr>
                                        @endif
                                    </table>
